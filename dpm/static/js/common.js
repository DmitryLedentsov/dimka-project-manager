(function ($) {
  ...
  window.DPM = {
    ...
    statusClass: function(status){status=(status||'unknown').toLowerCase();if(status==='running')return 'status-running';if(status==='failed'||status==='unhealthy'||status==='build_failed')return 'status-failed';if(status==='starting'||status==='stopping'||status==='restarting'||status==='deploying'||status==='building')return 'status-working';return 'status-stopped';},
