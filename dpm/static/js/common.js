(function ($) {
  ...
  window.DPM = {
    ...
    statusClass: function(status){status=(status||'unknown').toLowerCase();if(status==='running'||status==='ready')return 'status-running status-ready';if(status==='failed'||status==='unhealthy'||status==='build_failed')return 'status-failed';if(status==='degraded')return 'status-degraded';if(status==='starting'||status==='stopping'||status==='restarting'||status==='deploying'||status==='building'||status==='publishing'||status==='applying')return 'status-working status-'+status;if(status==='unpublished'||status==='empty')return 'status-stopped status-'+status;return 'status-stopped';},
  ...
  DPM.openModal=openModal;
  DPM.closeModals=closeModals;
